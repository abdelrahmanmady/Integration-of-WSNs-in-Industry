...
deployableLocations = creategrid2(2,3.5);
...
while (min(coverageLevels)< k) && (size(targetPointsTemp,1) ~=0) && (size(deployableLocations,1) ~=0) &&(maxdc~=0)
    ...
    for d=1:size(deployableLocations,1)
        dc=calculateDC2(deployableLocations(d,:),targetPointsTemp);
        if dc>maxdc
            maxdc=dc;
            index=d;
        end
...
for i=1:size(targetPoints,1)
    ...
    for j=1:size(deployed,1)-1
        distance = euclideanDistance2(targetPoints(i,:),deployed(j,:));
        if distance <= 5
    ...
    for j=1:size(coveringSensors,1)
        RSSI=checkRSSI2(targetPoints(i,:),coveringSensors(j,:));
