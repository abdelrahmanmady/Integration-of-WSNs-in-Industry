function [output] = updateCoverage(targetPoints,deployedLocation,coverageLevels)
    ...
    for i=1:size(targetPoints,1)
        distance = euclideanDistance(deployedLocation,targetPoints(i,:)); % distance between the deployed location to the target point.
        
        if (distance <= sensingRange) %if distance is less than sensing range and there exists los, consider it.
            vis=checkLOS(deployedLocation,targetPoints(i,:));
            if(vis==1)
                rssi=checkRSSI(deployedLocation,targetPoints(i,:));
                if(rssi>=-80)
